...
generate_simulation_templates <- function(m, p=2){
  ...
  dat<-AlgDesign::gen.factorial(desLevels, length(desLevels), varNames=desVarNames, center=TRUE)

  destT <- AlgDesign::optFederov(~., dat, nTrials = (n*(desOpt)), criterion="D")
  ...
  for(i in 1:m){
    ...
    list_df[[i]] <- data.frame(ID=1000+i,
                               ...
                               design_ouput)
    names(list_df[[i]]) <- c("ID", "ChoiceSet", "Choice", paste0(LETTERS[1:p], "_DCE"))
  }
  ...
  combinations <- combn(1:ncol(design_ouput2 ), 2)

  res <- matrix(0, nrow = nrow(design_ouput2)*ncol(combinations), ncol = l)

  k <- 1

  for(i in 1:nrow(design_ouput2)){
  ...
  for(i in 1:m){
    ...
    list_df[[i]] <- data.frame(ID=  rep(1000+i, 12),
                               ChoiceSet = rep(1:(nrow(design_ouput2)), each=desOpt2),
                               Choice = sample(c(rep(0, desOpt2-1), 1), desOpt2),
                               res
    )
    names(list_df[[i]]) <- c("ID", "ChoiceSet", "Choice", paste0(LETTERS[1:l], "_BW"))
  ...
  template_BW2 <- template_BW[, 1:(3+p)]


  processed_template_BW <- setUp(template_BW)
  processed_template_BW  <- remove_variables(processed_template_BW, processed_template_BW$attribute_names[length(processed_template_BW$attribute_names)])
...
#' @export
run_sims <- function(data_sets, chosen_values, precision_levels, integral_types, models, m_list, n_sims, p = 2, file = NULL){

  params <- list(data_sets = data_sets,
            ...
            for(i in 1:n_sims){
              ...
              for(g in integral_types){
                for(p in precision_levels[[g]]){
                  ...
                  big_list[[m_size]][[model_type]][[data_type]][[eg_name]][["results"]][[n_name]][[g]][[p_name]] <- estimate_model(big_list[[m_size]][[model_type]][[data_type]][[eg_name]][["results"]][[n_name]][["sim"]], g, p)
                }
              }
            }
            if(!is.null(file)){
...
#' @returns list of results
#' @export
process_sims <- function(big_list, data_sets, chosen_values, precision_levels, integral_types, models, m_list, n_sims, conf_level = 0.95){
