...
modelGraph <- function(object) {
  ...
  if (have_estimates) {
    mu_labels <- paste0(eta_nodes, ": ", greekLetters::greeks("mu"), "@_{", mus, "}", " = ", round(results$estimate[stringr::str_detect(estimated_params, "epsilon_")],3))
  } else {
  ...
  if (have_estimates) {
    beta_labels <-  paste0(greekLetters::greeks("beta"), "@_{",  betas[, 1], ", ", betas[, 2], "}", " = ", round(results$estimate[stringr::str_detect(estimated_params, "beta_")],3))
  } else {
  ...
  if(sum(model$delta[, 2] == 1) > 0){
    if (have_estimates) {
      gamma_labels <- paste0(greekLetters::greeks("sigma"),"@_{",  sigmas, "}", " = ", round(results$estimate[stringr::str_detect(estimated_params, "delta_sig_")],3))
    } else {
    ...
    if (have_estimates) {
      gamma_labels <-  paste0(greekLetters::greeks("gamma"), "@_{", gammas[, 1], ", ", gammas[, 2], "}", " = ", round(results$estimate[stringr::str_detect(estimated_params, "gamma_")],3))
    } else {
  ...
  edge_set_g <- DiagrammeR::create_edge_df(to = gammas[, 1]+length(zeta_nodes), from = gammas[, 2], rel = "gamma",  label = gamma_labels)
  ...
  if(nrow(betas) == 0){
    g <- DiagrammeR::set_node_attrs(g, node_attr = "fixedsize", values = F)
...
# model_graph(res_fixed) %>% DiagrammeR::render_graph()
# model_graph(model_fixed) %>% DiagrammeR::render_graph()
#
#
#
# model_graph(res_mtmm) %>% DiagrammeR::render_graph()
# model_graph(model_mtmm) %>% DiagrammeR::render_graph()
#
#
#
# model_graph(res_1f) %>% DiagrammeR::render_graph()
# model_graph(model_1f) %>% DiagrammeR::render_graph()
#
#
# model_graph(model_random) %>% DiagrammeR::render_graph()
# model_graph(res_random) %>% DiagrammeR::render_graph()
