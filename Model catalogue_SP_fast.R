library(DCM)
datamatrixfilename<-'/Users/uqajon14/Downloads/R Code_RP SP/RP.txt'
nmax_choiceset_size<-31
data_name<-'RP.RData'
data_matrix<-readData(datamatrixfilename)
...
ncovariates<-dim(concept_list$concept)[2]
lcovariates<-array(0, ncovariates)
for (i1 in 1:ncovariates) {lcovariates[i1]=paste('Cov',i1)}
#print(lcovariates)

processed=list(datamatrix=concept_list$datamatrix,datamatrixfilename=datamatrixfilename,data=concept_list$data,ncovariates=concept_list$ncovariates,nmaxchoicesetsize=nmax_choiceset_size,ndecisionmakers=ndecisionmakers,concept=concept_list$concept,fdd=fdd)
#print(processed)
save(file=data_name,list='processed')
print('Dimensions of raw datamatrix')
print(dim(concept_list$datamatrix))
print('Dimensions of process data: should be ndecisionmakers by nmaxchoicesetsize+4')
print(dim(concept_list$data))
print('Dimensions of concept matrix')
print(dim(concept_list$concept))
source('jessyfixed.R')
